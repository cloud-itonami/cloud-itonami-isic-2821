(ns agmachmfg.registry
  "Pure-function domain logic for the agricultural/forestry-machinery
  plant-operations coordination actor -- equipment/batch verification,
  shipment-quantity recompute, product-type validation, PTO no-load-
  speed plausibility validation, defect-rate plausibility validation,
  ...
  pre-existing `kotoba-lang/agmachmfg`-style capability library to wrap
  (verified: no such repo exists). The domain logic therefore lives
  here as pure functions, re-verified INDEPENDENTLY by
  `agmachmfg.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `texmachmfg.registry/shipment-quantity-exceeded?` from
  `cloud-itonami-isic-2826`, and `powertoolmfg.registry` from
  ...
  of issuing a machinery safety certification mark (this actor NEVER
  does any of those -- see README `What this actor does NOT do`).

  SCOPE: ISIC 2821 covers manufacture of agricultural and forestry
  machinery -- tractor-component, harvester and tillage-equipment
  assembly and testing lines producing finished tractors, combine
  harvesters, tillage implements, balers and forestry harvesters. This
  ...
  flagging, shipment coordination) -- it never touches the assembly/
  test-bench equipment directly, and it never stands in for the
  certification body that issues machinery safety compliance marks
  (e.g. CE marking under the EU Machinery Directive, or ROPS
  certification under the OECD Standard Codes for the Official Testing
  of Agricultural and Forestry Tractors).")
...
(def valid-product-types
  ...
  through."
  #{:tractor :combine-harvester :tillage-implement
    :baler :forestry-harvester})

(def pto-no-load-speed-rpm-min
  "Physical floor for a batch's own power-take-off (PTO) no-load test
  speed reading, in rpm. A no-load test speed is never negative."
  0.0)

(def pto-no-load-speed-rpm-max
  "Physical ceiling for a batch's own power-take-off (PTO) no-load test
  speed reading, in rpm. Informed by the ISO 500 series (Agricultural
  tractors -- Rear-mounted power take-off types 1, 2, 3 and 4) nominal
  PTO speeds of 540 rpm (540E) and 1000 rpm (1000E), and by the general
  mechanical/hydraulic-safety framework the ISO 4254 series
  (Agricultural machinery -- Safety) establishes for this equipment
  class -- this ceiling generously bounds well above any real routine
  no-load test on any standardized machine in this vertical (including
  self-propelled combine-harvester and forestry-harvester drivetrain
  test runs); a reading above it is implausible sensor/QC data, not a
  real test."
  1200.0)
...
(defn batch-verified?
  ...
  (i.e. its product-type/PTO-no-load-speed/quantity/defect-rate claims
...
(defn pto-no-load-speed-rpm-valid?
  "Is `rpm` a physically plausible power-take-off (PTO) no-load test
  speed reading, in rpm? Rejects nil, non-numbers, negative values, and
  values beyond `pto-no-load-speed-rpm-max` -- a fabricated or
  sensor-error reading, never let through as a real test-result fact."
  [rpm]
  (and (number? rpm)
       (>= (double rpm) pto-no-load-speed-rpm-min)
       (<= (double rpm) pto-no-load-speed-rpm-max)))
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a machinery safety certification mark -- this
...
(defn register-maintenance
  ...
  the RECORD a plant coordinator would keep. `agmachmfg.governor`
  ...
  actuate the equipment (see README `Actuation`), before this is ever
  allowed to commit."
...
(defn register-shipment
  ...
  outbound agricultural/forestry-machinery shipment against a
  ...
  coordinator would keep. `agmachmfg.governor` independently
  re-verifies the shipment's own claimed quantity against `shipment-
  quantity-exceeded?`, before this is ever allowed to commit."
