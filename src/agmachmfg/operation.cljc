(ns agmachmfg.operation
  "AgMachOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (AgMachAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Agricultural and Forestry
  Machinery Plant Operations Governor (:govern) and the rollout phase
...
(defn build
  "Compiles an AgMachOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `agmachmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AgMachAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Agricultural and Forestry Machinery Plant Operations Governor -- independent censor.
      (g/add-node :govern
