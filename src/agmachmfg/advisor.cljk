(ns agmachmfg.advisor
  "AgMachAdvisor -- the *contained intelligence node* for the
  agricultural/forestry-machinery (tractors, combine harvesters,
  tillage implements, balers and forestry harvesters) plant-operations
  coordination actor.

  It normalizes production-batch patches (product-type/PTO-no-load-
  speed/quantity/defect-rate), drafts an assembly/test-bench-equipment
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound agricultural/forestry-
  machinery shipment coordination proposal against a production batch.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real assembly/test-bench-equipment
  actuation, freight dispatch, or machinery safety certification --
  see README `What this actor does NOT do`. Every output is censored
  downstream by `agmachmfg.governor` before anything touches the SSoT.
  ...
  `agmachmfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [kotoba.lang.text :as str]
            [agmachmfg.registry :as registry]
...
(defn- log-production-batch
  ...
  PTO no-load speed, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft an assembly/test-bench-equipment maintenance-window scheduling
  ...
  `agmachmfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a mechanical-safety/hydraulic-hazard/PTO-guard-compliance
  ...
  technicality). See `agmachmfg.phase`: no phase ever adds this op to
  a phase's `:auto` set; `agmachmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound agricultural/forestry-machinery shipment
  ...
  invent one, and `agmachmfg.governor` NEVER trusts it: it
...
(def ^:private system-prompt
  (str "あなたは農業機械・林業機械(トラクター・コンバイン・耕耘機・"
       "ベーラー等の農業/林業生産用機械)製造工場"
       ...
       "トラクター・コンバイン・耕耘機等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "機械安全認証(CE/ROPS等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `agmachmfg.governor`
