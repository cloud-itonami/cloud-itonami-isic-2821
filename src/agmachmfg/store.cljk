(ns agmachmfg.store
  "SSoT for the agricultural/forestry-machinery plant-operations
  ...
  Scope note: like its siblings (`cloud-itonami-isic-2826`'s own
  `texmachmfg.store`, `cloud-itonami-isic-2818`'s own
  ...
  is self-contained (no external agricultural/forestry-machinery-
  manufacturing capability library, no jurisdiction-scoped Datomic-
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/PTO-no-load-speed/quantity/
    ...
    - `equipment`         -- a tractor-assembly-line or harvester-
                             test-bench unit's own record. `:verified?`/
    ...
    - `maintenance`       -- a scheduled assembly/test-bench-equipment
                             maintenance-window DRAFT against a piece
                             of equipment (`agmachmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound agricultural/forestry-
                             machinery shipment DRAFT
                             (`agmachmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (agmachmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (agmachmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :tractor
                :model "AGT-95H" :pto-no-load-speed-rpm 540.0
                :quantity-units 1000.0 :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-units 200.0
                :last-assessed "2026-07-01"}
   "batch-002" {:id "batch-002" :product-type :combine-harvester
                :model "CMB-6200" :pto-no-load-speed-rpm 1000.0
                :quantity-units 300.0 :defect-rate-percent 0.9
                :verified? true :registered? true
                :shipped-units 280.0
                :last-assessed "2026-07-01"}
   "batch-003" {:id "batch-003" :product-type :forestry-harvester
                :model "FHV-780" :pto-no-load-speed-rpm 800.0
                :quantity-units 5000.0 :defect-rate-percent 2.0
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-06-15"}})

(defn- sample-equipment []
  {"trac-001" {:id "trac-001" :kind :tractor-assembly-line
               :verified? true :registered? true
               :last-maintenance-date "2026-06-01"}
   "harv-002" {:id "harv-002" :kind :harvester-test-bench
               :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `agmachmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `agmachmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  tractor-assembly-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered harvester-test-bench unit (blocks any
