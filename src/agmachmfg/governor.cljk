(ns agmachmfg.governor
  "Agricultural and Forestry Machinery Plant Operations Governor -- the
  independent compliance layer that earns the AgMachAdvisor the right
  ...
  secretly tries to self-issue a machinery safety certification mark
  (an authority this actor never holds), whether a shipment proposal's
  ...
  `:agricultural-forestry-machinery-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       mark (e.g. CE marking under the
                                       EU Machinery Directive, or ROPS
                                       certification under the OECD
                                       Standard Codes) -- an authority
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`agmachmfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through.
   11. Invalid PTO no-load speed   -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:pto-no-load-speed-rpm` that is
                                       not a physically plausible
                                       power-take-off no-load test
                                       speed reading
                                       (`agmachmfg.registry/
                                       pto-no-load-speed-rpm-valid?`),
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [agmachmfg.registry :as registry]
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はトラクター・コンバイン・耕耘機等の試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "トラクター・コンバイン・耕耘機等の試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a machinery safety certification mark (e.g. CE marking
  under the EU Machinery Directive, or ROPS certification under the
  OECD Standard Codes) -- an authority exclusively reserved to the
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "機械安全認証(CE/ROPS等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-pto-no-load-speed-violations
  "For `:log-production-batch`, if the patch declares a
  `:pto-no-load-speed-rpm` that is not a physically plausible
  power-take-off no-load test speed reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rpm (:pto-no-load-speed-rpm (:value proposal))]
      (when (and (some? rpm) (not (registry/pto-no-load-speed-rpm-valid? rpm)))
        [{:rule :invalid-pto-no-load-speed
          :detail (str rpm "rpm は物理的に妥当な PTO 無負荷試験速度の範囲外")}]))))
...
(defn check
  "Censors an AgMachAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-pto-no-load-speed-violations request proposal)
