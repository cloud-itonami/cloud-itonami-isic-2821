(ns agmachmfg.governor-contract-test
  ...
  operations... does NOT self-issue a machinery safety certification
  mark') implemented faithfully. The single invariant under test:

    AgMachAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Agricultural and Forestry
    Machinery Plant Operations Governor would reject;
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :tractor}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :tractor (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the machinery safety certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-pto-no-load-speed-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:pto-no-load-speed-rpm 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-pto-no-load-speed} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:pto-no-load-speed-rpm (store/batch db "batch-001"))) "fabricated PTO no-load-speed never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "trac-001" :severity :moderate
                                            :description "PTO shaft guard vibration anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "trac-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
