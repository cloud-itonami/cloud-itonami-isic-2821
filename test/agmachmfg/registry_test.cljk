(ns agmachmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [agmachmfg.registry :as r]))
...
(deftest missing-quantity-is-not-flagged-exceeded
  ...
  (is (false? (r/shipment-quantity-exceeded? {:quantity-units 300.0} nil))))

;; ----------------------------- product-type-valid? -----------------------------

(deftest known-product-types-are-valid
  (doseq [pt [:tractor :combine-harvester :tillage-implement
...
(deftest typical-pto-no-load-speed-is-valid
  (is (r/pto-no-load-speed-rpm-valid? 0.0))
  (is (r/pto-no-load-speed-rpm-valid? 540.0))
  (is (r/pto-no-load-speed-rpm-valid? 1000.0))
  (is (r/pto-no-load-speed-rpm-valid? 1200.0)))

(deftest negative-pto-no-load-speed-is-invalid
  (is (not (r/pto-no-load-speed-rpm-valid? -1.0))))

(deftest excessive-pto-no-load-speed-is-invalid
  (is (not (r/pto-no-load-speed-rpm-valid? 999999.0)))
  (is (not (r/pto-no-load-speed-rpm-valid? 1200.01))))

(deftest non-numeric-or-missing-pto-no-load-speed-is-invalid
  (is (not (r/pto-no-load-speed-rpm-valid? nil)))
  (is (not (r/pto-no-load-speed-rpm-valid? "600"))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "trac-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "trac-001" 1)
