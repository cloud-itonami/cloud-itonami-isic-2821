(ns agmachmfg.operation-test
  "Smoke tests for the compiled AgMachOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `agmachmfg.governor-contract-test`; the Store contract in
  `agmachmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "AgMachOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "trac-001" :severity :moderate :description "PTO shaft guard vibration anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
